...
#   timestamp: 2026-05-18T23:00:05+00:00
...
class NebulaResultsConnectorConnectResponse(BaseModel):
    results: ConnectorConnectResponse


class NebulaResultsConnectorSyncResponse(BaseModel):
...
class NebulaResultsConnectorConnectionResponse(BaseModel):
    results: ConnectorConnectionResponse


class NebulaResultsConnectorDisconnectResponse(BaseModel):
    results: ConnectorDisconnectResponse


class NebulaResultsEngram(BaseModel):
    results: Engram


class NebulaResultsPresignedUploadResponse(BaseModel):
    results: PresignedUploadResponse


class NebulaResultsSnapshotImportResult(BaseModel):
...
class NebulaResultsListConnectorConnectionResponse(BaseModel):
...
class WrappedEngramsResponse(BaseModel):
    """
    ...
    applied_wal_seq: int | None = Field(
        0,
        description="Highest WAL committed sequence number reflected in this response. Non-zero only when the request was served via the WAL-tail fast path (collection-scoped requests on shards with WAL-tail compaction enabled). Zero on the legacy overlay path; clients should treat zero as 'the served path does not honor RYW assertions on this shard.' Pair with `min_applied_wal_seq` on the request to assert read-your-writes against a value returned by a prior memory-create call.",
        title='Applied Wal Seq',
    )
    results: list[Engram] = Field(..., title='Results')
    total_entries: int = Field(..., title='Total Entries')
...
class NebulaResultsSnapshotEnvelope(BaseModel):
...
class MemoryCreateResponse(
    RootModel[
        NebulaResultsMemoryCreateAcceptedResponse | NebulaResultsSnapshotMutationResult
    ]
):
    root: (
        NebulaResultsMemoryCreateAcceptedResponse | NebulaResultsSnapshotMutationResult
